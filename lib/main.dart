  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      theme: ThemeData(
        ...
        // state is not lost duringC the reload. To reset the state, use hot
        // restart instead.
        //
...
class _WeatherScreenState extends State<WeatherScreen> {
          ...
          children: [
            TextField(
              ...
              decoration: InputDecoration(
                labelText: 'Enter Location as  City Name ',
              ),
            ),
  ...
  void fetchWeatherData(String location) async {
    final apiKey = '48b79a810a59fdf86b8f47cb0663d865';
    //final url = 'https://api.openweathermap.org/data/2.5/weather?q=$location&appid=$apiKey';
   final url='http://api.openweathermap.org/data/2.5/weather?q=$location&APPID=$apiKey';
    final response = await http.get(Uri.parse(url));
    if (response.statusCode == 200) {
      final jsonData = json.decode(response.body);
      final mainWeather = jsonData['weather'][0]['main'];
      final description = jsonData['weather'][0]['description'];
      final main = jsonData['main'] as Object;
